...
from model.layers import HypergraphConv
from torch_geometric.transforms import LaplacianLambdaMax

class MSTHGNNBlock(nn.Module):
    ...
    def __init__(
        ...
        super(MSTHGNNBlock, self).__init__()

        self.hyperconv = HypergraphConv(in_channels, 256, symdegnorm=True, use_attention=False)
    ...
    def forward(
        ...
        if not isinstance(edge_index, list):
            ...
            for t in range(num_of_timesteps):
                ...
                x = X[:, :, :, t].reshape(batch_size * num_of_vertices, in_channels)
                output = self.hyperconv(x, edge_index[t])
                output = output.reshape(batch_size, num_of_vertices, -1)
                X_tilde.append(torch.unsqueeze(output, -1))

            X_tilde = F.relu(torch.cat(X_tilde, dim=-1)) # (B, N, F, T)
...
class MSTHGNN(nn.Module):
    ...
    def __init__(
        ...
        super(MSTHGNN, self).__init__()

        self._blocklist = nn.ModuleList(
            [MSTHGNNBlock(args.in_channels, args.K, args.nb_chev_filter, args.nb_time_filter, args.time_strides)]
        )

        self._blocklist.extend(
            [
                MSTHGNNBlock(args.nb_time_filter, args.K, args.nb_chev_filter, args.nb_time_filter, 1)
                for _ in range(args.nb_block - 1)
            ]
        )
    ...
    def forward(
            ...
            X = X.permute(0, 3, 1, 2) # (B, timestep, N_nodes, num_feature)
            X.permute(0, 3, 1, 2)
            X = self._final_conv(X)[:, :, :, -1].permute(0, 2, 1)
            return X