                ...
                for a in "${alpha[@]}"; do
                    for wd in "${weight_decay[@]}"; do
                        for ks in "${kernal_size[@]}"; do
                            sbatch --export=hidden_channels=$hc,lr=$l,alpha=$a,weight_decay=$wd,kernal_size=$ks,timestep_hidden=$timestep_hidden,location_aware=$location_aware experiment/run_uva.sh
                        done
                    done
                done
            done
